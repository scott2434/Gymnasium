...
#register this humanoid_standup-v5-2 environment so that it can be ran in sb3.py
register(
    # unique identifier for the env `name-version`
    id="humanoid-standup-v5-2.0",
    # path to the class for creating the env
    # Note: entry_point also accept a class as input (and not only a string)
    entry_point="humanoid-standup-v5-2:humanoidStandup_V5_2_Env",
...
class humanoidStandup_V5_2_Env(gym.Env):
   metadata = {"render_modes": ["human"], 'render_fps': 1}
        

r"""class HumanoidStandupEnv(MujocoEnv, utils.EzPickle):
...
"""
    ...
    metadata = {
        "render_modes": [
            "human",
            "rgb_array",
            "depth_array", # this comment contains origional humanoidstandup environment class(v5-2 is experimental rewrite SB_6/11) 
            "rgbd_tuple",
        ],
    } """
...
def __init__(
    ...
    **kwargs,
        ...
        self.action_space = Box(low=-0.4, high=0.4, shape=(15,), dtype=np.float32)
...
if __name__=="__main__":
    env = gym.make('humanoid-standup-v5-2.0') #, render_mode='human')
